...
public class HttpCmdExecutor implements Runnable {
	...
	public void run() {
		...
		try {
			...
			sendError(HTTP_INTERNALERROR,
					JSON.toJSONString(HttpCmdResponse.newResponse(false, "Error:" + t.getMessage())), false);
		}
	}
	...
	protected HttpCmdRequest resolveRequest(String uri, Properties params) {

		HttpCmdRequest request = new HttpCmdRequest();
		String[] pathNode = uri.substring(1, uri.length()).split("/");
		
		if (null == pathNode || pathNode.length < 2) {
			sendError(HTTP_BADREQUEST, "BAD REQUEST: path error");
		}
