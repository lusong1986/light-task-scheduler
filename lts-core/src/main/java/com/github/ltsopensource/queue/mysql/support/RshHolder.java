...
public class RshHolder {
	...
	public static final ResultSetHandler<List<JobLogPo>> JOB_LOGGER_LIST_RSH = new ResultSetHandler<List<JobLogPo>>() {
		...
		public List<JobLogPo> handle(ResultSet rs) throws SQLException {
			...
			while (rs.next()) {
				...
				final Map<String, String> extParams = JSON.parse(rs.getString("ext_params"),
						new TypeReference<Map<String, String>>() {
						});
				if (extParams != null) {
					// 从extParams拿到gray
					if (StringUtils.isNotEmpty(extParams.get(Constants.GRAY))) {
						jobLogPo.setGray(extParams.get(Constants.GRAY));
					}

					extParams.remove(Constants.GRAY);// 去除加的gray参数
				}
				jobLogPo.setExtParams(extParams);

				jobLogPo.setInternalExtParams(JSON.parse(rs.getString("internal_ext_params"),
						new TypeReference<HashMap<String, String>>() {
						}));
	...
	public static final ResultSetHandler<List<JobGrayPo>> JOB_GRAY_PO_LIST_RSH = new ResultSetHandler<List<JobGrayPo>>() {
		@Override
		public List<JobGrayPo> handle(ResultSet rs) throws SQLException {
			List<JobGrayPo> jobGrayPos = new ArrayList<JobGrayPo>();
			while (rs.next()) {
				JobGrayPo jobGrayPo = new JobGrayPo();
				jobGrayPo.setId(rs.getLong("id") + "");
				jobGrayPo.setTaskId(rs.getString("task_id"));
				jobGrayPo.setGray(rs.getString("gray"));
				jobGrayPo.setGmtCreated(formatter.format(new Date(rs.getLong("gmt_created"))));
				jobGrayPo.setGmtModified(formatter.format(new Date(rs.getLong("gmt_modified"))));
				jobGrayPos.add(jobGrayPo);
			}
			return jobGrayPos;
		}
	};
	...
	public static final ResultSetHandler<JobGrayPo> JOB_GRAY_PO_RSH = new ResultSetHandler<JobGrayPo>() {
		@Override
		public JobGrayPo handle(ResultSet rs) throws SQLException {
			if (!rs.next()) {
				return null;
			}
			JobGrayPo jobGrayPo = new JobGrayPo();
	...
	public static final ResultSetHandler<List<JobDependencyPo>> JOB_DEPENDENCY_PO_LIST_RSH = new ResultSetHandler<List<JobDependencyPo>>() {
		@Override
		public List<JobDependencyPo> handle(ResultSet rs) throws SQLException {
			List<JobDependencyPo> jobDependencyPos = new ArrayList<JobDependencyPo>();
			while (rs.next()) {
				JobDependencyPo jobDependencyPo = new JobDependencyPo();
				jobDependencyPo.setId(rs.getString("id"));
				jobDependencyPo.setJobId(rs.getString("job_id"));
				jobDependencyPo.setParentJobId(rs.getString("parent_job_id"));
				jobDependencyPo.setTaskTrackerNodeGroup(rs.getString("task_tracker_node_group"));
				jobDependencyPos.add(jobDependencyPo);
			}
			return jobDependencyPos;
		}
	};
}
