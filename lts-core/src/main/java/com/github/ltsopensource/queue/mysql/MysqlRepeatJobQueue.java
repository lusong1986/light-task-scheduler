...
public class MysqlRepeatJobQueue extends MysqlSchedulerJobQueue implements RepeatJobQueue {
	...
	public JobPo getJob(String jobId) {
		return new SelectSql(getSqlTemplate()).select().all().from().table(getTableName()).where("job_id = ?", jobId)
	...
	public JobPo getJob(String taskTrackerNodeGroup, String taskId) {
		...
		return new SelectSql(getSqlTemplate()).select().all().from().table(getTableName()).where("task_id = ?", taskId)
				.and("task_tracker_node_group = ?", taskTrackerNodeGroup).single(RshHolder.JOB_PO_RSH);
	}
	...
	public int incRepeatedCount(String jobId) {
		while (true) {
			...
			if (new UpdateSql(getSqlTemplate()).update().table(getTableName())
					.set("repeated_count", jobPo.getRepeatedCount() + 1).where("job_id = ?", jobId)
					.and("repeated_count = ?", jobPo.getRepeatedCount()).doUpdate() == 1) {
