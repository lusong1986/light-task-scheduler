 ...
 * | 1 | 2 | 3 | 4 | 5 | 6 | 7 |
 *
 * 1、大端4个字节整数，等于2、3、4、5、6, 7长度总和 1、大端4个字节整数，等 serializable id 3、header 信息长度 大端4个字节整数，等于3的长度 4、header 信息内容 5、body 信息长度
 * 大端4个字节整数，等于5的长度 6、body 信息内容 7、body 的class名称
 */
public class DefaultCodec extends AbstractCodec {
	...
	public RemotingCommand decode(ByteBuffer byteBuffer) throws Exception {
		...
		RemotingSerializable serializable = getRemotingSerializable(serializableId);
		...
		if (remaining > 0) {
			...
			if (bodyLength > 0) {
				...
				cmd.setBody((RemotingCommandBody) serializable.deserialize(bodyData,
						Class.forName(new String(bodyClassData))));
			}
		}
		return cmd;
	}
	...
	public ByteBuffer encode(RemotingCommand remotingCommand) throws Exception {
		...
		RemotingSerializable serializable = getRemotingSerializable(remotingCommand.getSid());
