...
public abstract class AbstractRemoting {
	...
	public Pair<RemotingProcessor, ExecutorService> getDefaultRequestProcessor() {
	...
	public void processRequestCommand(final Channel channel, final RemotingCommand cmd) {
		...
		if (pair != null) {
								...
								try {
									channel.writeAndFlush(response).addListener(new ChannelHandlerListener() {
										...
										public void operationComplete(Future future) throws Exception {
											if (!future.isSuccess()) {
												LOGGER.error(
														"response to " + RemotingHelper.parseChannelRemoteAddr(channel)
																+ " failed", future.cause());
			...
			final RemotingCommand response = RemotingCommand.createResponseCommand(
					RemotingProtos.ResponseCode.REQUEST_CODE_NOT_SUPPORTED.code(), error);
			response.setOpaque(cmd.getOpaque());
			channel.writeAndFlush(response);
				...
				channel.writeAndFlush(request).addListener(new ChannelHandlerListener() {
					...
					public void operationComplete(Future future) throws Exception {
						if (future.isSuccess()) {
							...
							LOGGER.warn("call invokeAsyncImpl future.cause():" + future.cause());
							CatUtils.recordEvent(false, channel.remoteAddress().toString(), "INVOKE_ASYNC");
							responseFuture.setSendRequestOK(false);
						}
