...
public final class BeanCopierFactory {
	...
	public static <Source, Target> BeanCopier<Source, Target> createCopier(Class<?> sourceClass, Class<?> targetClass,
		...
		try {
			Class<?> beanCopierClazz = COMPILER.compile(getClassCode(sequence, sourceClass, targetClass, deepCopy,
					propCvtMap));
				...
				if (setMethod != null) {
					...
					if (propCvtMap != null && propCvtMap.containsKey(field.getName())) {
						...
						methodCode.append(converterType).append(" ").append(converterName).append(" = (")
								...
								.append(".getConverter(").append(sequence).append(",").append("\"")
								.append(field.getName()).append("\");\n");
						methodCode.append("target.").append(setMethod.getName()).append("(").append("(")
								.append(targetFieldClass.getName()).append(")").append(converterName)
								.append(".convert(").append("source").append(")").append(");\n");
						continue;
					}
					...
					if (getMethod.getReturnType() == targetFieldClass) {
						if (!deepCopy) {
							...
							if (ClassHelper.isPrimitiveType(targetFieldClass)
								...
								methodCode.append("target.").append(setMethod.getName()).append("(").append("(")
										.append(targetFieldClass.getName()).append(")")
										.append(BeanUtils.class.getName()).append(".deepClone(").append("source.")
										.append(getMethod.getName()).append("()").append(")").append(");\n");
							}
						}
						...
						methodCode.append("source.").append(getMethod.getName()).append("() == null ? ")
								.append(String.valueOf(ClassHelper.getPrimitiveDftValue(targetFieldClass)))
								.append(" : ").append("source.").append(getMethod.getName()).append("()");
						methodCode.append(");\n");
