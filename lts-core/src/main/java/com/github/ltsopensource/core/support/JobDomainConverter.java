...
public class JobDomainConverter {
	...
	public static JobPo convert(Job job) {
		...
		if (CollectionUtils.isNotEmpty(job.getJobDependencies())) {
			List<JobDependencyPo> jobDependencyPos = new ArrayList<JobDependencyPo>();
			for (JobDependency jobDependency : job.getJobDependencies()) {
			...
			jobPo.setJobDependencyPos(jobDependencyPos);
		}
