...
/**
 * @author Robert HG (254963746@qq.com) on 8/18/14. 抽象客户端
 */
public abstract class AbstractClientNode<T extends Node, Context extends AppContext> extends
		AbstractJobNode<T, Context> {
	...
	protected void remotingStart() {
		...
		if (defaultProcessor != null) {
			...
			remotingClient.registerDefaultProcessor(
					defaultProcessor,
	...
	private RemotingClient getRemotingClient(RemotingClientConfig remotingClientConfig) {
		return ServiceLoader.load(RemotingTransporter.class, config)
				.getRemotingClient(appContext, remotingClientConfig);
	}
