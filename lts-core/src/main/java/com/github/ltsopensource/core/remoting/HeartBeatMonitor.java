...
public class HeartBeatMonitor {
	...
	public HeartBeatMonitor(RemotingClientDelegate remotingClient, AppContext appContext) {
		...
		this.jobTrackerUnavailableEventSubscriber = new EventSubscriber(HeartBeatMonitor.class.getName() + "_PING_"
				+ appContext.getConfig().getIdentity(), new Observer() {
			@Override
			public void onObserved(EventInfo eventInfo) {
				CatUtils.recordEvent(true, eventInfo.getTopic(), "JOBTRACKER_UNAVAILABLE_EVENT");
				startFastPing();
				stopPing();
			}
		});
		...
		appContext.getEventCenter().subscribe(
				new EventSubscriber(HeartBeatMonitor.class.getName() + "_NODE_ADD_"
						+ appContext.getConfig().getIdentity(), new Observer() {
					@Override
					public void onObserved(EventInfo eventInfo) {
	...
	private void check(Node jobTracker) {
		...
		if (beat(remotingClient, jobTracker.getAddress())) {
			...
			stopFastPing();
			startPing();
			CatUtils.recordEvent(true, jobTracker.getIdentity(), "CHECK_JOBTRACKER_AVAILABLE");
		} else {
			CatUtils.recordEvent(false, jobTracker.getIdentity(), "CHECK_JOBTRACKER_UNAVAILABLE");
	...
	private boolean beat(RemotingClientDelegate remotingClient, String addr) {
		...
		try {
			...
			if (response != null
					&& JobProtos.ResponseCode.HEART_BEAT_SUCCESS == JobProtos.ResponseCode.valueOf(response.getCode())) {
