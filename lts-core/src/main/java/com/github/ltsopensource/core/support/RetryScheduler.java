...
/**
 * @author Robert HG (254963746@qq.com) on 8/19/14. 重试定时器 (用来发送 给 客户端的反馈信息等)
 */
public abstract class RetryScheduler<T> {
	...
	public RetryScheduler(String name, final AppContext appContext, String storePath) {
		...
		EventSubscriber subscriber = new EventSubscriber(RetryScheduler.class.getSimpleName().concat(
				appContext.getConfig().getIdentity()), new Observer() {
			@Override
	...
	public void start() {
		try {
			if (selfCheckStart.compareAndSet(false, true)) {
				this.RETRY_EXECUTOR_SERVICE = Executors.newSingleThreadScheduledExecutor(new NamedThreadFactory(
						"LTS-RetryScheduler-retry", true));
				// 这个时间后面再去优化
				scheduledFuture = RETRY_EXECUTOR_SERVICE.scheduleWithFixedDelay(new CheckSelfRunner(), 10, 30,
						TimeUnit.SECONDS);
				LOGGER.info("Start {} RetryScheduler success, identity=[{}]", name, appContext.getConfig()
						.getIdentity());
			}
		} catch (Throwable t) {
	...
	private void startMasterCheck() {
		try {
			if (masterCheckStart.compareAndSet(false, true)) {
				this.MASTER_RETRY_EXECUTOR_SERVICE = Executors.newSingleThreadScheduledExecutor(new NamedThreadFactory(
						"LTS-RetryScheduler-master-retry", true));
				// 这个时间后面再去优化
				masterScheduledFuture = MASTER_RETRY_EXECUTOR_SERVICE.scheduleWithFixedDelay(
						new CheckDeadFailStoreRunner(), 30, 60, TimeUnit.SECONDS);
				LOGGER.info("Start {} master RetryScheduler success, identity=[{}]", name, appContext.getConfig()
						.getIdentity());
			}
		} catch (Throwable t) {
			LOGGER.error("Start {} master RetryScheduler failed, identity=[{}]", name, appContext.getConfig()
					.getIdentity(), t);
		}
	}
	...
	private void stopMasterCheck() {
		try {
			if (masterCheckStart.compareAndSet(true, false)) {
				...
				LOGGER.info("Stop {} master RetryScheduler success, identity=[{}]", name, appContext.getConfig()
						.getIdentity());
			}
		} catch (Throwable t) {
			LOGGER.error("Stop {} master RetryScheduler failed, identity=[{}]", name, appContext.getConfig()
					.getIdentity(), t);
		}
	}
			...
			try {
				...
				for (FailStore store : failStores) {
					...
					while (true) {
						...
						if (retry(values)) {
							LOGGER.info(
									"{} RetryScheduler, dead local files send success, identity=[{}], size: {}, {}",
									name, appContext.getConfig().getIdentity(), values.size(),
									JSON.toJSONString(values));
							store.delete(keys);
						} else {
				...
				LOGGER.error("Run {} master RetryScheduler error, identity=[{}] ", name, appContext.getConfig()
						.getIdentity(), e);
			} finally {
	...
	public void inSchedule(String key, T value) {
		try {
			...
			LOGGER.info("{} RetryScheduler, local files save success, identity=[{}], {}", name, appContext.getConfig()
					.getIdentity(), JSON.toJSONString(value));
		} catch (FailStoreException e) {
			LOGGER.error("{} RetryScheduler in schedule error, identity=[{}]", name, e, appContext.getConfig()
					.getIdentity());
		} finally {
			if (lock.isHeldByCurrentThread()) {
				lock.unlock();
