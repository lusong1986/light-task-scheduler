...
public abstract class AbstractJobNode<T extends Node, Context extends AppContext> implements JobNode {
	...
	private void initHttpCmdServer() {
		// 命令中心
		int port = appContext.getConfig().getParameter(ExtConfig.HTTP_CMD_PORT, Constants.DEFAULT_HTTP_COMMAND_PORT);
	...
	final public void stop() {
		try {
			...
			LOGGER.error("========== Stop failed, nodeType={}, identity={}", config.getNodeType(),
					config.getIdentity(), e);
		}
	}
