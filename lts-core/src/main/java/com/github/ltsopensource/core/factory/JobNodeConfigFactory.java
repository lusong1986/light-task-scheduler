...
public class JobNodeConfigFactory {
	...
	public static void buildIdentity(Config config) {
		if (config.getNodeType().equals(NodeType.JOB_TRACKER) || config.getNodeType().equals(NodeType.TASK_TRACKER)
				|| config.getNodeType().equals(NodeType.MONITOR)) {
			config.setIdentity(getNodeTypeShort(config.getNodeType()) + "_" + config.getIp() + "_" + getPid());
			return;
		}
