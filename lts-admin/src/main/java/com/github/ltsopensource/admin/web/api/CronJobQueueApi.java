...
public class CronJobQueueApi extends AbstractMVC {
		...
		try {
			...
			if (nextTriggerTime == null) {
				return Builder.build(false,
						StringUtils.format("该CronExpression={} 已经没有执行时间点! 请重新设置或者直接删除。", request.getCronExpression()));
			}
			...
			if (success) {
				...
				try {
					...
					boolean relyOnPrevCycleChanged = !newJobPo.getRelyOnPrevCycle().equals(
							oldJobPo.getRelyOnPrevCycle());
					...
					if (oldJobPo.getRelyOnPrevCycle() && !relyOnPrevCycleChanged) {
						...
						if (relyOnPrevCycleChanged) {
							...
							if (oldJobPo.getRelyOnPrevCycle()) {
								...
								try {
									newJobPo.setInternalExtParam(Constants.EXE_SEQ_ID,
											JobUtils.generateExeSeqId(newJobPo));
			...
			CatUtils.recordEvent(true, oldJobPo.getTaskId(), "UPDATE");
	...
	public RestfulResponse cronJobDelete(JobQueueReq request) {
		...
		CatUtils.recordEvent(true, jobPo.getTaskId(), "DELETE");
	...
	@RequestMapping("/job-queue/cron-job-suspend-all")
	public RestfulResponse cronJobSuspendAll(JobQueueReq request) {
