...
public class RepeatJobQueueApi extends AbstractMVC {
	...
	private ExecutorService suspendThreadPool = Executors.newFixedThreadPool(10);
		...
		if (success) {
			try {
				...
				boolean repeatIntervalOrCountDecChanged = repeatIntervalChanged
						|| ((oldJobPo.getRepeatCount() == -1 && newJobPo.getRepeatCount() > 0) || (oldJobPo
								.getRepeatCount() != -1 && newJobPo.getRepeatCount() != -1 && newJobPo.getRepeatCount() < oldJobPo
								.getRepeatCount()));
				if (oldJobPo.getRelyOnPrevCycle() && !relyOnPrevCycleChanged) {
					...
					if (relyOnPrevCycleChanged) {
						if (oldJobPo.getRelyOnPrevCycle()) {
							...
							appContext.getExecutableJobQueue().remove(oldJobPo.getTaskTrackerNodeGroup(),
									oldJobPo.getJobId());
							appContext.getNoRelyJobGenerator().generateRepeatJobForInterval(newJobPo, new Date());
						} else {
							// 之前不依赖,现在依赖,需要删除批量任务
							appContext.getExecutableJobQueue().removeBatch(oldJobPo.getRealTaskId(),
									oldJobPo.getTaskTrackerNodeGroup());
							// 添加新的任务
						...
						// 如果relyOnPrevCycle 没有修改过, 表示relyOnPrevCycle=false,
						// 那么要看repeatIntervalOrCountDecChanged,如果修改过,需要删除重新生成
			...
			CatUtils.recordEvent(true, oldJobPo.getTaskId(), "UPDATE_REPEAT_JOB");
			JobLogUtils.log(LogType.UPDATE, oldJobPo, appContext.getJobLogger());
			return Builder.build(true);
		} else {
			CatUtils.recordEvent(false, oldJobPo.getTaskId(), "UPDATE_REPEAT_JOB");
	...
	public RestfulResponse repeatJobDelete(JobQueueReq request) {
		...
		if (success) {
			try {
				...
				LOGGER.error(e.getMessage(), e);
				CatUtils.recordEvent(false, jobPo.getTaskId(), "DEL_REPEAT_JOB");
		...
		CatUtils.recordEvent(true, jobPo.getTaskId(), "DEL_REPEAT_JOB");
	...
	@RequestMapping("/job-queue/repeat-job-suspend-all")
	public RestfulResponse repeatJobSuspendAll(JobQueueReq request) {
		final int limit = 100;
		...
		for (final JobPo jobPo : repeatJobs) {
			Future<String> future = suspendThreadPool.submit(new Callable<String>() {
		...
		return Builder.build(true, "suspend all job succeed");
	}
