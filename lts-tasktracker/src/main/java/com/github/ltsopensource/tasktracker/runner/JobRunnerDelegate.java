...
public class JobRunnerDelegate implements Runnable {
	...
	public void run() {
		...
		try {
			...
			final String messageId = jobMeta.getInternalExtParam(Cat.Context.CHILD + 1);
			final String parentId = jobMeta.getInternalExtParam(Cat.Context.PARENT);
			final String rootId = jobMeta.getInternalExtParam(Cat.Context.ROOT);
			...
			transaction = CatUtils.catTransaction("JOB_RUNNER", jobMeta.getRealTaskId());
			...
			while (jobMeta != null) {
				...
				try {
					...
					jobJson.put("jobStartTime", startTime);
					jobJson.put("jobId", jobMeta.getJobId());
					jobJson.put("realTaskId", jobMeta.getRealTaskId());
					jobJson.put("taskTrackerNodeGroup", jobMeta.getJob().getTaskTrackerNodeGroup());
					jobJson.put("triggerTime", jobMeta.getJob().getTriggerTime());
					transaction.addData("jobInfo", jobJson.toJSONString());
				} catch (Exception e) {
					LOGGER.warn("addData jobInfo warn", e);
				}
