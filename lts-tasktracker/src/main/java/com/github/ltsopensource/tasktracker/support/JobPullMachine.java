...
/**
 * 用来向JobTracker去取任务 1. 会订阅JobTracker的可用,不可用消息主题的订阅 2. 只有当JobTracker可用的时候才会去Pull任务 3. Pull只是会给JobTracker发送一个通知
 *
...
public class JobPullMachine {
	...
	private void sendRequest() throws RemotingCommandFieldCheckException {
		...
		if (availableThreads == 0) {
			CatUtils.recordEvent(false, appContext.getConfig().getIdentity(), "NO_AVAILABLE_THREADS");
			return;
		}
		...
		final String group = System.getenv(Constants.GROUP);
		if (StringUtils.isNotEmpty(group)) {
			requestBody.putExtParam(Constants.GROUP, group);
		}
