...
public class ClientNotifier {
	...
	private boolean send0(String nodeGroup, final List<JobRunResult> results) {
		...
		final String finalTaksId = taskId;
		...
		RemotingCommand commandRequest = RemotingCommand.createRequestCommand(
				JobProtos.RequestCode.JOB_COMPLETED.code(), requestBody);

		final Transaction transaction = CatUtils.catTransaction("PUSH_TO_JOBCLIENT", finalTaksId);

		final Holder<Boolean> result = new Holder<Boolean>();
		try {
			final CountDownLatch latch = new CountDownLatch(1);
			getRemotingServer().invokeAsync(jobClientNode.getChannel().getChannel(), commandRequest,
					new AsyncCallback() {
						@Override
						public void operationComplete(ResponseFuture responseFuture) {
							try {
								...
								if (commandResponse != null
										&& commandResponse.getCode() == JobProtos.ResponseCode.JOB_NOTIFY_SUCCESS
												.code()) {
									...
									result.set(false);
									CatUtils.recordEvent(false, finalTaksId, "JOB_NOTIFY_FAILED");
								}
							} finally {
								latch.countDown();
							}
