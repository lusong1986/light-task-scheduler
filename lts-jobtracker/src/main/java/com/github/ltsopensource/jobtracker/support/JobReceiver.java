...
import com.github.ltsopensource.store.jdbc.exception.SameTaskNameException;
...
public class JobReceiver {
	...
	public void receive(JobSubmitRequest request) throws JobReceiveException {
		...
		for (Job job : jobs) {
			try {
				...
				if (exception == null) {
					exception = new JobReceiveException(e.getMessage(), e);
				}
	...
	private JobPo addToQueue(Job job, JobSubmitRequest request) {
		...
		try {
			...
			// 不允许同一个tasktracker group下面创建同名job
			checkSameNameJob(job);
	...
	private void checkSameNameJob(Job job) {
		JobPo existJobPo = appContext.getExecutingJobQueue().getJob(job.getTaskTrackerNodeGroup(), job.getTaskId());
		if (existJobPo == null) {
			existJobPo = appContext.getExecutableJobQueue().getJob(job.getTaskTrackerNodeGroup(), job.getTaskId());
			if (existJobPo == null) {
		...
		if (existJobPo != null) {
			LOGGER.warn("Job already exist, cannot creat. nodeGroup={}, {}", job.getTaskTrackerNodeGroup(), job);
			throw new SameTaskNameException("这个执行节点组下面已经有同名task存在，请更换");
		}
	}
	...
	private void addJob(Job job, JobPo jobPo) throws DupEntryException {
		// add to job gray flag
		if (StringUtils.hasText(jobPo.getGray())) {
			appContext.getJobGrayFlag().add(jobPo.getTaskId(), jobPo.getGray());
		}

		// 先添加依赖关系
		if (CollectionUtils.isNotEmpty(jobPo.getJobDependencyPos())) {
			for (JobDependencyPo jobDependencyPo : jobPo.getJobDependencyPos()) {
				appContext.getExecutableJobDependency().add(jobPo.getJobId(), jobDependencyPo);
			}
		} else {
			appContext.getExecutableJobDependency().add(jobPo.getJobId());
		}
	...
	private void addTriggerTimeJob(JobPo jobPo) {
		...
		if (needAdd2ExecutableJobQueue) {
			...
			JobTraceUtil.setCatMessageIds(jobPo);
