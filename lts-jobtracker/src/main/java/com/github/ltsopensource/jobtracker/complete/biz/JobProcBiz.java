...
public class JobProcBiz implements JobCompletedBiz {
	...
	public RemotingCommand doBiz(JobCompletedRequest request) {
		...
		JobMeta firstJobPo = results.get(0).getJobMeta();
		String messageId = firstJobPo.getInternalExtParam(Cat.Context.CHILD + 1);
		String parentId = firstJobPo.getInternalExtParam(Cat.Context.PARENT);
		String rootId = firstJobPo.getInternalExtParam(Cat.Context.ROOT);
		...
		final Transaction transaction = CatUtils.catTransaction("JOB_COMPLETE_BIZ", taskId);
		...
		try {
			if (CollectionUtils.sizeOf(results) == 1) {
				...
				String taskids = "";
				for (JobRunResult result : results) {
					taskids += result.getJobMeta().getRealTaskId() + "|";
				}
				CatUtils.recordEvent(true, taskids, "MULTIRESULTS_PROCESS");
	...
	private boolean needRetry(JobRunResult result) {
		// 判断类型
		if (!(Action.EXECUTE_LATER.equals(result.getAction()) || Action.EXECUTE_EXCEPTION.equals(result.getAction()))) {
			return false;
		}
		...
		boolean needRetry = !(retryTimes >= globalMaxRetryTimes || retryTimes >= jobMaxRetryTimes);
		if (needRetry) {
			CatUtils.recordEvent(true, result.getJobMeta().getRealTaskId(), "NEED_RETRY_CHECK");
		}
		return needRetry;
	}
