	...
	public void start() {
		...
		try {
			executorService = Executors.newScheduledThreadPool(1, new NamedThreadFactory(
					NonRelyOnPrevCycleJobScheduler.class.getSimpleName(), true));
	...
	private void schedule() {
		...
		while (true) {
			...
			for (JobPo jobPo : jobPos) {
				...
				CatUtils.recordEvent(true, jobPo.getTaskId(), "NONRELYONPREV_ADDCRONJOB_FORINTERVAL");
		...
		while (true) {
			...
			for (JobPo jobPo : jobPos) {
				...
				CatUtils.recordEvent(true, jobPo.getTaskId(), "NONRELYONPREV_ADDREPEATJOB_FORINTERVAL");
