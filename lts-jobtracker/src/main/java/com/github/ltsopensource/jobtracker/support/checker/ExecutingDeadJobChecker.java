...
/**
 * @author Robert HG (254963746@qq.com) on 8/19/14. 死掉的任务 1. 分发出去的，并且执行节点不存在的任务 2. 分发出去，执行节点还在, 但是没有在执行的任务
 */
public class ExecutingDeadJobChecker {
	...
	public void start() {
		try {
			if (start.compareAndSet(false, true)) {
				int fixCheckPeriodSeconds = appContext.getConfig().getParameter(
						ExtConfig.JOB_TRACKER_EXECUTING_JOB_FIX_CHECK_INTERVAL_SECONDS, 30);
	...
	private void checkAndFix() throws RemotingSendException {
		...
		int maxDeadCheckTime = appContext.getConfig().getParameter(
				ExtConfig.JOB_TRACKER_EXECUTING_JOB_FIX_DEADLINE_SECONDS, 20);
	...
	private void askTimeoutJob(Channel channel, final List<JobPo> jobPos) {
		try {
			...
			RemotingCommand request = RemotingCommand.createRequestCommand(JobProtos.RequestCode.JOB_ASK.code(),
					requestBody);
				...
				public void operationComplete(ResponseFuture responseFuture) {
					...
					if (response != null && RemotingProtos.ResponseCode.SUCCESS.code() == response.getCode()) {
						...
						if (CollectionUtils.isNotEmpty(deadJobIds)) {
							...
							QuietUtils.sleep(appContext.getConfig().getParameter(
									ExtConfig.JOB_TRACKER_FIX_EXECUTING_JOB_WAITING_MILLS, 1000L));
	...
	private void fixDeadJob(JobPo jobPo) {
		...
		try {
			...
			final String messageId = jobPo.getInternalExtParam(Cat.Context.CHILD + 1);
			final String parentId = jobPo.getInternalExtParam(Cat.Context.PARENT);
			final String rootId = jobPo.getInternalExtParam(Cat.Context.ROOT);
			if (StringUtils.isNotEmpty(messageId) && StringUtils.isNotEmpty(parentId) && StringUtils.isNotEmpty(rootId)) {
				CatUtils.buildContext(messageId, parentId, rootId);
			}

			transaction = CatUtils.catTransaction("JOB_FIX_DEAD", jobPo.getRealTaskId());
			...
			CatUtils.recordEvent(true, jobPo.getTaskId(), "FIX_EXECUTING_DEADJOB");
			CatUtils.catSuccess(transaction);
			stat.incFixExecutingJobNum();
		} catch (Throwable t) {
			if (transaction != null) {
				CatUtils.catException(transaction, t);
			}
